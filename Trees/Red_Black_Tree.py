...
class Node:
    def __init__(self, val):
        ...
        self.color = 1  # New node is always inserted as RED Node
...
class RBTree:
    ...
    def insertNode(self, key):
        ...
        y = None
        x = self.root  # x là root

        while x != self.NULL:  # Find position for new node
            y = x  # y
            if node.val < x.val:
                x = x.left  # Xuống node con trái
            else:
    ...
    def delete_node_helper(self, node, key):
        ...
        if z.left == self.NULL:  # If left child of z is NULL
            ...
            y = self.minimum(z.right)  # Find minimum of the right subtree
